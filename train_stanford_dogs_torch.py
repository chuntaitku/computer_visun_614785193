...
import numpy as np
import matplotlib
matplotlib.use('Agg') # Saves plots to disk without requiring a window
...
class StanfordDogsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Read image via OpenCV and convert to RGB
...
def get_model(model_name):
    if model_name == "ResNet50":
        ...
        # Replace final fully connected head
...
try:
    # Build a fresh EfficientNet structure and reload weights
    eval_model = get_model("EfficientNetB0")
    eval_model.load_state_dict(torch.load("best_EfficientNetB0.pth"))
    eval_model.eval()
    ...
    # Grab a sample tensor from the evaluation dataset
    ...
    # Recover original image for plotting visual contrast
    # Reverse ImageNet scaling to make it clean color array again
